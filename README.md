...
bats（bash 用テストフレームワーク）で **27 ファイル・347 テスト**を網羅している。
...
| `statusline-session-scope.bats` | 45 | bg 集計のセッションスコープ・生きている bg 判定（agent symlink + lsof 保持・#53）・ターン状態読み取り・バナー/タブタイトル生成・AppleScript 注入除去 (#42) |
...
├── tests/                    # bats テスト 27 ファイル・347 テスト
...
│   └── hooks/test_enforce_subagent_progress.bats / hooks/test_turn_state.bats
├── launchd/
